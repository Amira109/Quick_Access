...
{
  $id = $_GET['id'];
  $get_books = "select * from sol_books WHERE id = '$id'";
...
{ 
  ...
  $sql = "UPDATE `sol_books` SET `name`='$name',`description`='$description',`image`='$image',`semester`='$semester',`year`='$year',`url_download`='$url_download' WHERE id = '$btn_id'" ;
  $res = mysqli_query($con , $sql);

  if($res){
    echo "<script>alert('Edit Done')</script>";
...
<body>
  <br>
  <h2 style="text-align: center;">Edit Solution Book</h2>
  <br>
